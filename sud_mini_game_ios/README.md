...
Next, download the iOS client code from [here](), and modify the following information:

- Modify the authentication information in the `GameViewController.swift` file.
- Modify the `Bundle Identifier` of the demo, which needs to be consistent with the `Bundle Identifier` provided when obtaining authentication information.
...
├── MiniGameDemo                            ---- Project files
|   ├── MiniGameDemo                        ---- Project source code file
│           ├── GameViewController.swift    ---- Game room logic code file
|   ├── MiniGameDemo.xcodeproj              ---- Project file
│   ├── MiniGameDemo.xcworkspace            ---- Project file
│   ├── Podfile                             ---- Pod file
│   ├── Podfile.lock                        ---- Pod file
│   ├── Pods                                ---- Pods package
│   ├── SudMGPWrapper                       ---- Sud API wrapper
│   ├── SudMGPWrapper.podspec               ---- Sud API wrapper project pod file
```
