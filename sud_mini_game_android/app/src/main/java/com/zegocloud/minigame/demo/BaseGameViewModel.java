...
public abstract class BaseGameViewModel implements SudFSMMGListener {
    ...
    private void loadGame(Activity activity, String code, long gameId) {
        ...
        SudLoadMGParamModel paramModel = new SudLoadMGParamModel();
        paramModel.userId = getUserId();
        paramModel.roomId = getGameRoomId();
        paramModel.code = code;
        paramModel.mgId = gameId;
        paramModel.language = getLanguageCode();
        paramModel.activity = activity;

        // 调用游戏sdk加载游戏
        // Invoke the game SDK to load the game.
        ISudFSTAPP iSudFSTAPP = SudMGP.loadMG(paramModel, sudFSMMGDecorator);
